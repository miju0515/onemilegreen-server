package orgmeanapi.onemilegreen.Service;


import orgmeanapi.onemilegreen.Domain.MileEffect;
import orgmeanapi.onemilegreen.Domain.UserRoutine;
...
public class RoutineService {
    ...
    public void userRoutineUpdate(int user_rou_id){
        ...
        this.jdbcTemplate.update("update UserRoutine set ur_status=2,ur_fin_time=? where user_rou_id=?",
                java.sql.Timestamp.valueOf(date),user_rou_id);
    }
    ...
    public void insertRoutine(String fileUrl,String content,int user_rou_id){
        ...
        this.jdbcTemplate.update("insert into UserRoutineDetail(user_rou_id,urd_cre_time,urd_image,urd_content,urd_useYN,urd_status) values(?,?,?,?,1,1)",
                user_rou_id,java.sql.Timestamp.valueOf(date),fileUrl,content);
    }
    ...
    public void addMileEffect(int user_rou_id){
        ...
        String district=jdbcTemplate.queryForObject("select district from User where user_no=?",String.class,user_no);
        ...
        MileEffect uMileEffect=jdbcTemplate.queryForObject("select user_mileage,user_effect from User where user_no=?", new RowMapper<MileEffect>() {
            @Override
            public MileEffect mapRow(ResultSet rs, int rowNum) throws SQLException {
                return new MileEffect(
        ...
        this.jdbcTemplate.update("update User set user_mileage=?,user_effect=? where user_no=?",
    ...
    public List<UserRoutine> findAll(int rou_id){
        List<UserRoutine> results=jdbcTemplate.query(
                "SELECT U.user_no,U.user_nick,UR.rou_id,URD.urd_image,URD.urd_like " +
                        "FROM User U " +
